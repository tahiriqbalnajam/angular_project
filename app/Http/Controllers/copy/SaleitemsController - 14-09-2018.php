...
//			$new_price = $request->input('quantity') * $request->input('price');
	...
	{
		$query = DB::table('sale_items_received')->where('sale_items_received.deleted','no')->where('sale_items_received.item_id',$id);
				$query->join('sale_items', 'sale_items.id', '=', 'sale_items_received.item_id');
			$data['data'] = $query->get(array('sale_items_received.*','sale_items.name'));
			return response($data);
	}
