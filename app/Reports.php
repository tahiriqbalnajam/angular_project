...
{
	public function get_direct_item_detail($start_date='',$end_date = '',$item_id = '')
	{
		return DB::table('items')->where('items.id',$item_id)
										->whereBetween('rs.date', [$start_date, $end_date])->where('rs.deleted','no')
										->join('running_sale_items as rs', 'rs.item_id', '=', 'items.id')->get();
	}
	public function running_sale_items() {
    	return $this->hasMany('App\Running_sale_items');
	}
